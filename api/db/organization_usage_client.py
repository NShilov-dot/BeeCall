...
class OrganizationUsageClient(BaseDBClient):
    ...
    def _calculate_current_period(
        ...
        if org.quota_type == "monthly":
            ...
            # Clamp reset_day to the last day of the target month — e.g. reset_day=31
            # in February should resolve to Feb 28/29, not raise ValueError.
            def _clamped(target: datetime) -> datetime:
            ...
            if now.day >= reset_day:
                period_start = _clamped(now)
            else:
                # Previous month
                period_start = _clamped(now - relativedelta(months=1))
