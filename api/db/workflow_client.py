...
class WorkflowClient(BaseDBClient):
    ...
    async def _assert_workflow_in_org(
        self, session, workflow_id: int, organization_id: int
    ) -> None:
        """Verify workflow belongs to the given org; raise ValueError if not.

        Used by version/draft methods that operate on workflow_id but must
        enforce tenant isolation — the FK constraint only proves the row
        exists, not that the caller is allowed to touch it.
        """
        result = await session.execute(
    ...
    async def save_workflow_draft(
        ...
        """
        async with self.async_session() as session:
            await self._assert_workflow_in_org(session, workflow_id, organization_id)
    ...
    async def get_all_workflows(
        ...
        if user_id is None and organization_id is None:
            raise ValueError(
                "get_all_workflows requires organization_id (preferred) or user_id "
                "for tenant scoping; an unscoped listing would leak cross-tenant data."
            )
        async with self.async_session() as session:
    ...
    async def get_all_workflows_for_listing(
        self, organization_id: int, status: str = None
    ) -> list[WorkflowModel]:
        ...
        Args:
            organization_id: Filter by organization ID (required; unscoped
                listing would leak cross-tenant data)
    ...
    async def get_workflow_counts(self, organization_id: int) -> dict[str, int]:
        """Get workflow counts by status.

        Args:
            organization_id: Filter by organization ID (required; unscoped
                counts would mix tenants)
    ...
    async def update_workflow(
        ...
        Raises:
            ValueError: If the workflow with the given ID is not found, or if
                neither organization_id nor user_id is provided.
        """
        if user_id is None and organization_id is None:
            raise ValueError(
                "update_workflow requires organization_id (preferred) or user_id "
                "for tenant scoping; an unscoped update would let a caller modify "
    ...
    async def get_workflow_name(
        ...
        Scoping is mandatory: pass ``organization_id`` (preferred) or
        ``user_id``. An unscoped lookup would let a request-supplied id reveal
        another tenant's workflow name.
        """
        if user_id is None and organization_id is None:
            raise ValueError(
                "get_workflow_name requires organization_id (preferred) or user_id "
                "for tenant scoping."
            )
        async with self.async_session() as session:
    ...
    async def get_workflow_run_count(
    ...
    async def update_definition_node_summaries(
        self, definition_id: int, organization_id: int, node_summaries: dict
    ) -> None:
        ...
        Args:
            ...
            organization_id: The org that owns the parent workflow — required
                so a runtime/QA task can't poke another tenant's definition by
                guessing an id.
            node_summaries: Dict mapping node_id to summary data
                (e.g. {"summary": "...", "trace_url": "..."})
        """
        async with self.async_session() as session:
            # Join definition→workflow to verify org ownership in a single query.
            result = await session.execute(
                select(WorkflowDefinitionModel)
                .join(
                    WorkflowModel,
    ...
    async def get_workflow_run_counts(
        self, workflow_ids: list[int], organization_id: int
    ) -> dict[int, int]:
        ...
        Args:
            ...
            organization_id: Only workflows in this org are counted; ids for
                other orgs silently get 0 (treat as not found).
        ...
        async with self.async_session() as session:
            # Restrict to ids that actually belong to this org — joining
            # against WorkflowModel ensures cross-tenant ids return 0.
            result = await session.execute(
                select(
    ...
    async def add_call_disposition_code(
        self, workflow_id: int, organization_id: int, disposition_code: str
    ) -> None:
