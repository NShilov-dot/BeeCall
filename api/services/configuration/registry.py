...
BEECALL_PROVIDER_MODEL_CONFIG = provider_model_config(
    "BeeCall",
    description=(
        "In-house OpenAI-compatible LLM. Set base_url to your gateway's "
        "/v1 endpoint. Streaming, function calling, and multimodal inputs "
...
# Default placeholder — operators override per-org via /settings. Listed here
# so the UI dropdown has at least one suggestion before any model is wired up.
BEECALL_LLM_MODELS = ["default"]


@register_llm
class BeeCallLLMConfiguration(BaseLLMConfiguration):
    model_config = BEECALL_PROVIDER_MODEL_CONFIG
    provider: Literal[ServiceProviders.BEECALL] = ServiceProviders.BEECALL
    model: str = Field(
        default="default",
        description="Model identifier exposed by the BeeCall LLM endpoint.",
        json_schema_extra={"examples": BEECALL_LLM_MODELS, "allow_custom_input": True},
    )
    base_url: str = Field(
        default="https://llm.beecall.internal/v1",
        description=(
            "OpenAI-compatible base URL — must point at the /v1 root, not at "
            "/v1/chat/completions. The AsyncOpenAI client appends the path."
        ),
    )
    api_key: str | list[str] | None = Field(
        default=None,
        description=(
            "Bearer token sent as `Authorization: Bearer ...`. Leave blank if "
            "the gateway authenticates via network policy (mTLS / VPC)."
        ),
    )
