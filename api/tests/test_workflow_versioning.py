...
class TestWorkflowCreation:
    async def test_create_workflow_produces_published_v1(
        ...
        versions = await db_session.get_workflow_versions(workflow.id, organization_id=workflow.organization_id)
...
class TestPublishDraft:
    async def test_publish_promotes_draft_to_published(
        ...
        published = await db_session.publish_workflow_draft(workflow.id, organization_id=workflow.organization_id)
    ...
    async def test_publish_updates_released_pointer(self, db_session, workflow_with_v1):
        ...
        await db_session.publish_workflow_draft(workflow.id, organization_id=workflow.organization_id)
...
class TestDiscardDraft:
    async def test_discard_removes_draft(self, db_session, workflow_with_v1):
        ...
        await db_session.discard_workflow_draft(workflow.id, organization_id=workflow.organization_id)
...
class TestRevert:
    async def _publish_v2(self, db_session, workflow):
        ...
        return await db_session.publish_workflow_draft(workflow.id, organization_id=workflow.organization_id)
    ...
    async def test_revert_creates_draft_from_archived(
        ...
        versions_before = await db_session.get_workflow_versions(workflow.id, organization_id=workflow.organization_id)
        ...
        # Revert to V1
        draft = await db_session.revert_to_version(workflow.id, v1_id, organization_id=workflow.organization_id)
    ...
    async def test_revert_preserves_all_snapshot_fields(
        ...
        # Revert to V2
        draft = await db_session.revert_to_version(workflow.id, v2.id, organization_id=workflow.organization_id)
    ...
    async def test_revert_when_draft_exists_raises(self, db_session, workflow_with_v1):
        ...
        with pytest.raises(ValueError, match="[Dd]raft.*exists"):
            await db_session.revert_to_version(workflow.id, v1_id, organization_id=workflow.organization_id)
