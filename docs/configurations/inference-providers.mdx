...
### Piper: in-process TTS with no API key

TTS defaults to **Piper**, which synthesizes speech inside the API process instead of calling a cloud provider. It needs no API key and no network access, so a fresh agent speaks out of the box. Pick a voice from the `rhasspy/piper-voices` catalog (for example `ru_RU-irina-medium`); Russian voices are bundled and the field accepts any catalog id.

Voice models are baked into the API image at build time, because the runtime host may not be able to reach huggingface. To bundle different or additional voices, pass a comma-separated list at build time:

```bash
docker build --build-arg PIPER_VOICES=ru_RU-irina-medium,ru_RU-dmitri-medium -f api/Dockerfile .
```

Each voice adds roughly 63 MB to the image. Piper runs on CPU by default; enable **Use CUDA** only on a GPU host. Speech speed is not adjustable for this provider.
