...
export function setStackRefreshCookie(refreshToken: string) {
  ...
  const isBeeCallDomain = window.location.hostname.endsWith('.beecall.com');
  const cookieDomainPart = isBeeCallDomain ? '; domain=.beecall.com' : '';
...
export async function impersonateAsSuperadmin(params: {
  ...
  // (served from the target sub-domain, e.g. app.beecall.com). The route will set the
  ...
  // superadmin.beecall.com we want to switch to app.beecall.com. For any other domain
